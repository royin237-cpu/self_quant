...
OUTPUT_EXCEL = r"F:\self_quant\quant\回测excel\小市值低价轮动_上市满1年过滤.xlsx"
OUTPUT_IMAGE = r"F:\self_quant\quant_gemini\csv\small_cap_min_listing_1y.png"
...
def run_backtest():
    ...
    # ---------------- 新增：计算每只股票的“首次出现日期” ----------------
    print("正在计算股票首次出现日期以识别上市时间...")
    first_date_df = df.groupby('code')['date'].min().reset_index()
    first_date_df.columns = ['code', 'first_date']
    # ------------------------------------------------------------------
    ...
    # 合并首次出现日期
    df = pd.merge(df, first_date_df, on='code', how='left')
    ...
    print("开始按周执行【小市值+低价+上市满1年】调仓模拟...")
    ...
    for date in trade_dates:
        ...
        # ---------------- 核心逻辑修改：过滤上市不满一年的股票 ----------------
        # 1. 2010年即存在的股票直接视为符合条件 (first_date.year <= 2010)
        # 2. 其余股票需满足：当前日期 - 首次出现日期 > 365天
        valid_pool = daily_data[
            (daily_data['tradestatus'] == 1) &
            (daily_data['isST'] == 0) &
            (daily_data['peTTM'] > 0) & (daily_data['pbMRQ'] > 0) &
            (
                (daily_data['first_date'].dt.year <= 2010) | 
                ((date - daily_data['first_date']).dt.days > 365)
            )
        ].copy()
        # ---------------------------------------------------------------------
    ...
    ax1 = plt.subplot(211)
    ax1.plot(df_assets.index, df_assets['total_asset'], label='策略净值', color='green')
    ax1.set_title(f"小市值+上市满1年过滤 (总收益: {total_return*100:.2f}%)")
    ax1.set_ylabel("总资产")
    ax1.grid(True)
    ax2 = plt.subplot(212, sharex=ax1)
    ax2.fill_between(df_assets.index, 0, df_assets['drawdown'], color='orange', alpha=0.3, label='回撤')
